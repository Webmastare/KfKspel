...
function displayLogs() {
    ...
    for (let i = gameLogs.length-1; i >= 0; i--) {
        ...
        let wrap = getLogString(log);
        loggingContainer.appendChild(wrap);
    }
}
function getLogString(log) {
    const wrapper = document.createElement('div');
    let head;
    if (userPlayer && log.playerID == userPlayer.playerID) {
        head = `<span>${formatTimestamp(log.timestamp)}</span> - <span style="background-color: #f58989;">${log.playerID}</span>`;
    } else {
        head = `<span>${formatTimestamp(log.timestamp)}</span> - <span>${log.playerID}</span>`;
    }
    ...
    switch (log.action) {
        case "shot": {
            if (userPlayer && log.details.targetUser == userPlayer.playerID) {
                string += ` sköt <span style="background-color: #f58989;">${log.details.targetUser}</span>  (${log.details.targetUserLives} hjärtan kvar)`
            } else {
                string += ` sköt <span>${log.details.targetUser}</span>  (${log.details.targetUserLives} hjärtan kvar)`
            }
            break;
        }
        case "move": {
            string += ` flyttade från [${log.details.from.column}, ${log.details.from.row}] till [${log.details.to.column}, ${log.details.to.row}]`
            break;
        }
        case "range": {
...
async function doAction(action, moveDirection = null) {
    ...
    try {
        ...
        console.log(data.updatedData, data.shotData, data.detailsQuery);
        // Update user
        ...
        gameLogs.push(data.updatedLogs);
        displayLogs();
    } catch (error) {
        console.error('There was a problem with the fetch operation:', error);
    }
