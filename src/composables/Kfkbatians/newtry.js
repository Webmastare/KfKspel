...
import{ångerknapp, ånger} from './funktionerna/regret.js';
import { action, cardAction, längdfix, dubbelklick } from './funktionerna/action.js';
import { knappar, botton } from './funktionerna/andrabrum.js';
...
export class card extends GameObject
{
    ...
    static kortbredd = 45;
    static korthöjd = 70;
    static baksida = 'kortbilder/baksida.svg';
...
class kortlek
{
    ...
    fyllLeken(){
        ...
        for(let i = 0; i < 4; i++){
            for(let j = 0; j < 13; j++){
                let namnpåbild = 'kortbilder/' + färger[i] + '/'+ färger[i] + '_' + valörer[j] + '.svg';
...
export class spawnare extends GameObject
{
    ...
    static redoside = 'kortbilder/tomlek.svg';
...
const botten = new GameObject(0, höjd, bredd, 1000, 'coolkort.png');
...
export function StartaSpelet(){
    ...
    const hjärt = new behållare(bredd - 120, 100, 'hjärter', 'kortbilder/behållare/behållare_hjärter.svg');
    const rute = new behållare(bredd - 120, 190, 'ruter', 'kortbilder/behållare/behållare_ruter.svg');
    const klöv = new behållare(bredd - 120, 280, 'klöver', 'kortbilder/behållare/behållare_klöver.svg');
    const spade = new behållare(bredd - 120, 370, 'spader', 'kortbilder/behållare/behållare_spader.svg');
...
function LäggUtAllaSuperKort(){
    ...
    for(let i = 0; i < antalHögar; i++){
        let currentCard = new card(position.x, position.y, 'kortbilder/superkort.svg', '', 14, 0);
