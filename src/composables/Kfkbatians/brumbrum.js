...
import { Fusk, Clearing, klickad, validklick, Uskare } from './main.js'
import { botton, knappar } from './andrabrum.js'
...
class kontinuerliga {
  ...
  static drawGameObject(listan) {
    ...
    for (let i = 0; i < listan.length; i++) {
      ...
      // Only draw if image is loaded and not in error state
      if (nuvarande.bildLoaded && !nuvarande.bildError) {
        try {
          ctx.drawImage(
            nuvarande.bild,
            nuvarande.position.x,
            nuvarande.position.y,
            nuvarande.width,
            nuvarande.height,
          )
        } catch (error) {
          console.warn(
            `Error drawing image for object at position ${nuvarande.position.x}, ${nuvarande.position.y}:`,
            error,
          )
        }
      }
    ...
    for (let i = 0; i < listan.length; i++) {
      ...
      if (nuvarande.bildLoaded && !nuvarande.bildError) {
        try {
          ...
          console.warn(
            `Error drawing physics object at position ${nuvarande.position.x}, ${nuvarande.position.y}:`,
            error,
          )
        }
      }
...
class Animation {
  ...
  static GåIgnomAllaAnimationer() {
    ...
    for (let i = 0; i < animationslängden && rörelsebarn; i++) {
      ...
      let längdKvarVektor = Vector.SubtraheraVector(
        nuvarandeObjekt.position,
        nuvarande.målpositioner,
      )
...
class GameObject {
  ...
  constructor(xpos, ypos, bredd, höjd, bildLänk, off = 50) {
    ...
    this.bild = new Image()
    this.bildLoaded = false
    this.bildError = false

    // Add event listeners for image loading
    this.bild.onload = () => {
      this.bildLoaded = true
    }

    this.bild.onerror = () => {
      this.bildError = true
      console.warn(`Failed to load image: ${bildLänk}`)
    }
